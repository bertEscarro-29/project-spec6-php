...
if($_POST && isset($_POST['saveChanges'])){
    ...
    $dbStatement = $db->prepare('SELECT * FROM departments WHERE deptfullname = ? AND deptshortname = ?');
    $dbStatement->execute([
        $deptFullName,
        $deptShortName
    ]);
      if($dbStatement->rowCount() > 0){
            $_SESSION['errors']['deptFullName'] = "Same Data, Ignoring update to this field";
            $_SESSION['errors']['deptShortName'] = "Same Data, Ignoring update to this field";
            header("Location: $entryURL", true, 301);
            exit();
        }
    ...
    //check if department full name or short name already exists in the database
