...
if($_POST && isset($_POST['saveChanges'])){
    ...
    $dbStatement = $db->prepare('SELECT * FROM colleges WHERE collfullname = ? AND collshortname = ?');
    $dbStatement->execute([
        $schoolFullName,
        $schoolShortName
    ]);
     ...
     if($dbStatement->rowCount() > 0){
            $_SESSION['errors']['schoolFullName'] = "Same Data, Ignoring update to this field";
            $_SESSION['errors']['schoolShortName'] = "Same Data, Ignoring update to this field";
            header("Location: $entryURL", true, 301);
            exit();
        }
